...
public class Stinger extends Weapon {
	...
	public Stinger( Vehicle carAttachedTo ) {
		super( "Stinger" , carAttachedTo );
		// set all the default weapon specs
		mAmmoCount = 4;
		mRateOfFire = 0.6f;
//		soundComponent = new VehicleSound(null);
		range = 60;		// we can see 20 meters out, bullet can live longer
		lockOnSpeed = 0.4f;
		
		
		
		mSprite = new Sprite( Assets.Stinger );
		...
		mBody.setUserData( new EntityData( getID(), getOwnerID(), EntityData.Type.STINGER ) );
	}
	...
	public boolean fireNoLock() {
		...
		SeekerMissile missile = new SeekerMissile();
		missile.getBody().setUserData( new EntityData( missile.getID(), getOwnerID(), EntityData.Type.SEEKER_MISSILE ) );
//		bullet.setOwnerID( getOwnerID() );
		missile.setType( EntityData.Type.SEEKER_MISSILE );
		...
		missile.setTransform( startPoint.x, startPoint.y, angle );
    	
    	// Fire off the bullet in the direction pointed to by the radian angle    	
    	missile.setVelocity( -MathUtils.sin( angle ), MathUtils.cos( angle ) );
			  	
    	
		BattleScreen.getBattleStage().addActor( missile );	// Add the bullet to the stage
		BattleArena.getInstance().addProjectile( missile );
